...
a = int(input("\nЗакажите билеты на конференцию.\nВНИМАНИЕ: При покупке 3 и более билетов предоставляется скидка 10% на весь заказ.\nВведите количество билетов, которые Вы хотите приобрести: "))
print("")
for i in range(1, a+1):
    age = int(input(f"Введите возраст посетителя по билету №{i}: "))
    if age > 100 or age < 0:    # условие на случай введения пользователем некорректного возраста - меньше 0 или больше 100
        print("Введен некорректный возраст посетителя. Заказ этого билета отменен. Количество заказанных билетов будет скорректировано.\n")
        a = a - 1
    elif 0 <= age < 18:
        Order.append(age)
        Order_Sum += 0
...
if len(Order) and Adult >= 3:       #условие применения скидки - если заказ на более чем 3 человек и если заказ на более чем 3 взрослых
